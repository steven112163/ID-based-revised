...
do
    ...
    if [ $c -gt 0 ]; then
        for i in ${arr[@]}
        do
            mkdir -p /tmp/$i/out -m 777
            out_n=$(ls /tmp/$i/out | grep "^test" | wc -l)+1
            ...
            if [ $exists -eq 1 ]; then
                ...
                into outfile '/tmp/$i/out/test$out_n.csv' fields terminated by ',' enclosed by '\"' lines terminated by '\\n'"
            fi


            mkdir -p /tmp/$i/in -m 777
            in_n=$(ls /tmp/$i/in | grep "^test" | wc -l)+1
            ...
            if [ $exists -eq 1 ]; then
                mysql -uroot -p'root' portal -se "select Date, Time, (select Location from Switch where \
                Switch_ID = Dst_access_sw), Bytes from Association where Asso_ID > $s \
                and Asso_ID <= $e and Switch_ID = Dst_access_sw and Dst_User_ID = $i \
                into outfile '/tmp/$i/in/test$in_n.csv' fields terminated by ',' enclosed by '\"' lines terminated by '\\n'"
            fi
